...
3. **flash 审不出 → 交 pro(同样循环)**: flash 阶段收敛(无必须修/无新发现)后, 交给 pro 模型子代理做**全面独立审计**——pro 从头独立深挖全库(不依赖 flash 的结论; 自然会验证 flash 修复的正确性并找其遗漏的同类缺陷, 历史教训: flash 只修了 Windows 枚举失败分支的 UAF, pro 抓到成功路径的同类 UAF/double-free)。pro 同样循环迭代: pro 审计 → 修复 → 再派 pro 复审(分类并行) → 修复 → ..., 直到 pro 也报"无必须修/无新发现"(pro 收敛)。
